...
import { User, Package, ArrowLeft, Clock, Star } from 'lucide-react'
import { LandscapeBackground } from './landscape-background'
import { RequireCourier } from '@/components/auth/require-courier'
import { AddressesProvider } from '@/contexts/addresses-context'
import { ThemeToggle } from '@/components/ui/theme-toggle'
import { cn } from '@/lib/utils'

const NAV_ITEMS = [
  { to: '/courier/profile', icon: User, label: 'Профиль' },
  { to: '/courier/orders', icon: Package, label: 'Заказы' },
  { to: '/courier/history', icon: Clock, label: 'История' },
  { to: '/courier/reviews', icon: Star, label: 'Отзывы' },
]
  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-5 pb-4 border-b border-white/8">
        ...
        <p className="text-[11px] text-nav-text/40 mt-2 ml-9">Панель курьера</p>
      </div>
